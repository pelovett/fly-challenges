...
func (state *NodeState) getOffset(topic string) (int, error) {
	...
	curOffset, err := state.kv.ReadInt(ctx, topic)
	if err != nil {
	...
	for remainingTries > 0 {
		...
		defer cancel()
		// Get current max offset
		err = state.kv.CompareAndSwap(ctx, topic, curOffset, curOffset+1, true)
		if err != nil {
			if rpcErr, ok := err.(*maelstrom.RPCError); ok {
				switch rpcErr.Code {
...
func (state *NodeState) savePotential(topic string, offset int, msg int) {
	...
	// Check if we have potentials for that topic
	curMsgs, present := state.potentialMsgs[topic]
	if !present {
...
func (state *NodeState) startPublish(n *maelstrom.Node, topic string, msg int) (int, error) {
	...
	for _, node := range n.NodeIDs() {
		...
		n.RPC(
			node,
			map[string]any{"type": "promote_potential", "key": topic, "offset": offset},
			func(msg maelstrom.Message) error {
				var err error
				if msg.Type() != "promote_potential_ok" {
					err = errors.New("error failed to send potential")
					errChan <- err
				}
				done <- true
