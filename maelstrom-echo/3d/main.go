...
func heartbeat(n *maelstrom.Node, state *NodeState) {
	...
	for _, node := range state.topo[n.ID()] {
		n.RPC(node, map[string]any{"type": "echo"}, func(msg maelstrom.Message) error {
			...
			// If they haven't received our recent updates, send them over
			updateSignals, newLastProcessed := state.getSingleUpdateMap(node)
...
func main() {
	...
	n.Handle("broadcast", func(msg maelstrom.Message) error {
		...
		// Update state
		newSignalsPresent := nodeState.updateState(signals)

		// If any signals were new, broadcast messages to neighbors
		if newSignalsPresent {
			broadCastNewMsgs(msg.Src, n, &nodeState, logger)
		}
		return nil
	})
